...
from data import get_transform, get_dataset, SplitDataset, make_dataloader, get_inferen_data, \
    dataset_class_list,non_iid_dirichlet
...
def main():
    ...
    data_train_split, label_train_split = non_iid_dirichlet(dataset['train'], cfg['numbers'], cfg['alpha'])

    global_model = get_model(cfg['model_name'], dataset, mode_rate=1)
    layer_neuron_count = {
        'layer1.0.conv1.weight': torch.zeros(global_model.layer1[0].conv1.weight.shape[0])
    }
    ...
    for i in range(cfg['rounds']):
        ...
        for user_idx in user_idxs:
            ...
            load_params_to_client_model(global_model, client_model, inferen_data, cfg['mode'], device, rate,
                                        cfg['select_mode'], label_mask, clients_models_shape, user_idx, layer_neuron_count)
        ...
        accuracy, loss = run_global_test(global_model, dataset, device)
        scheduler.step()
        records.append({'Round': i + 1, 'Accuracy': accuracy, 'Loss': loss})

    print("\n=== Neuron Selection Counts ===")
    for layer_name, counts in layer_neuron_count.items():
