...
class Conv(nn.Module):
    ...
    def get_idx_layer_aware(self, input, rate, device, topmode):
        start_channels = (torch.arange(self.data_shape[0]))
        out = self.relu(self.norm(self.scaler(self.conv(input))))
        first_channels = self.get_layer_aware_top(out, 0, int(rate * self.hidden_size[0]), device, topmode)
        ...
        for s in seq_index:
            ...
            out = blc(out)
            second_channels = self.get_layer_aware_top(out, s, int(rate * self.hidden_size[s]), device, topmode)
    ...
    def get_layer_aware_top(self, input, name, k, device, topmode):
        # TODO()
        if name == 0:
            blc = self.layer1
            return get_diff_channels_sum(input, blc, k, device, topmode)
        elif name == 1:
            blc = self.layer2
            return get_diff_channels_sum(input, blc, k, device, topmode)
        elif name == 2:
    ...
    def get_idx_fixed_topk(self, rate, fixed_ratio=0.2, topmode='absmax'):
        ...
        total_conv = self.hidden_size[0]
        fixed = int(fixed_ratio * total_conv * rate)
        topk = int(rate * total_conv) - fixed

        all_indices = torch.arange(total_conv)
        fixed_channels = all_indices[:fixed]
        remaining = all_indices[fixed:]

        conv_weights = self.conv.weight.detach()
        scores = torch.sum(torch.abs(conv_weights[remaining]), dim=(1, 2, 3))
        topk_indices = get_top_index(scores, topk, topmode)
        first_channels = torch.cat([fixed_channels, remaining[topk_indices]])

        self.idx['conv.weight'] = (first_channels, start_channels)
        self.idx['conv.bias'] = first_channels
        self.idx['norm.weight'] = first_channels
        self.idx['norm.bias'] = first_channels

        for layer_idx in [1, 2, 3]:
            param_name = f'layer{layer_idx}.conv'
            total_channels = self.hidden_size[layer_idx]
            fixed = int(fixed_ratio * total_channels * rate)
            topk = int(rate * total_channels) - fixed

            all_indices = torch.arange(total_channels)
            fixed_channels = all_indices[:fixed]
            remaining = all_indices[fixed:]

            conv_layer = getattr(self, f'layer{layer_idx}').conv
            conv_weights = conv_layer.weight.detach()

            scores = torch.sum(torch.abs(conv_weights[remaining]), dim=(1, 2, 3))
            topk_indices = get_top_index(scores, topk, topmode)
            current_channels = torch.cat([fixed_channels, remaining[topk_indices]])

            self.idx[f'{param_name}.weight'] = (current_channels, first_channels)
            self.idx[f'{param_name}.bias'] = current_channels
            self.idx[f'layer{layer_idx}.norm.weight'] = current_channels
            self.idx[f'layer{layer_idx}.norm.bias'] = current_channels

            first_channels = current_channels.clone()
...
def get_diff_channels_sum(input, blc, k, device, topmode):
    res = []
