...
class Block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, scaler_rate=0.75, track=False):
    ...
    def get_idx_fixed_random(self, first_channels, rate, param_name, fixed_ratio=0.2):
        total_channels_conv1 = self.conv1.weight.shape[0]
        fixed_count_conv1 = int(fixed_ratio * total_channels_conv1 * rate)
        random_count_conv1 = int(rate * total_channels_conv1) - fixed_count_conv1


        all_indices_conv1 = torch.arange(total_channels_conv1)

        fixed_channels_conv1 = all_indices_conv1[fixed_count_conv1:fixed_count_conv1+fixed_count_conv1]

        remaining_channels_conv1 = torch.cat(
            (all_indices_conv1[:fixed_count_conv1], all_indices_conv1[fixed_count_conv1 + fixed_count_conv1:]))


        random_channels_conv1 = remaining_channels_conv1[
            torch.randperm(len(remaining_channels_conv1))[:random_count_conv1]
        ]

        second_channels = torch.cat((fixed_channels_conv1, random_channels_conv1))
        self.idx[param_name + '.conv1.weight'] = (second_channels, first_channels)
        self.idx[param_name + '.n1.weight'], self.idx[param_name + '.n1.bias'] = second_channels, second_channels

        total_channels_conv2 = self.conv2.weight.shape[0]
        fixed_count_conv2 = int(fixed_ratio * total_channels_conv2 * rate)
        random_count_conv2 = int(rate * total_channels_conv2) - fixed_count_conv2
        all_indices_conv2 = torch.arange(total_channels_conv2)
        fixed_channels_conv2 = all_indices_conv2[:fixed_count_conv2]
        remaining_channels_conv2 = all_indices_conv2[fixed_count_conv2:]
        random_channels_conv2 = remaining_channels_conv2[
            torch.randperm(len(remaining_channels_conv2))[:random_count_conv2]
        ]
        third_channels = torch.cat((fixed_channels_conv2, random_channels_conv2))
        self.idx[param_name + '.conv2.weight'] = (third_channels, second_channels)
        self.idx[param_name + '.n2.weight'], self.idx[param_name + '.n2.bias'] = third_channels, third_channels
        self.idx[param_name + '.shortcut.weight'] = (third_channels, first_channels)
    ...
    def get_idx_fixed_topk(self, first_channels, rate, param_name, fixed_ratio=0.2, topmode='absmax'):
        total_channels_conv1 = self.conv1.weight.shape[0]
        fixed_count_conv1 = int(fixed_ratio * total_channels_conv1 * rate)
        topk_count_conv1 = int(rate * total_channels_conv1) - fixed_count_conv1
        all_indices_conv1 = torch.arange(total_channels_conv1)
        fixed_channels_conv1 = all_indices_conv1[:fixed_count_conv1]
        remaining_channels_conv1 = all_indices_conv1[fixed_count_conv1:]
        conv1_scores = torch.sum(self.conv1.weight[remaining_channels_conv1], dim=(1, 2, 3))
        topk_channels_conv1 = remaining_channels_conv1[get_topk_index(conv1_scores, topk_count_conv1, topmode)]

        second_channels = torch.cat((fixed_channels_conv1, topk_channels_conv1))
        self.idx[param_name + '.conv1.weight'] = (second_channels, first_channels)
        self.idx[param_name + '.n1.weight'], self.idx[param_name + '.n1.bias'] = second_channels, second_channels

        total_channels_conv2 = self.conv2.weight.shape[0]
        fixed_count_conv2 = int(fixed_ratio * total_channels_conv2 * rate)
        topk_count_conv2 = int(rate * total_channels_conv2) - fixed_count_conv2

        all_indices_conv2 = torch.arange(total_channels_conv2)
        fixed_channels_conv2 = all_indices_conv2[:fixed_count_conv2]

        remaining_channels_conv2 = all_indices_conv2[fixed_count_conv2:]
        conv2_scores = torch.sum(self.conv2.weight[remaining_channels_conv2], dim=(1, 2, 3))
        topk_channels_conv2 = remaining_channels_conv2[get_topk_index(conv2_scores, topk_count_conv2, topmode)]

        third_channels = torch.cat((fixed_channels_conv2, topk_channels_conv2))
        self.idx[param_name + '.conv2.weight'] = (third_channels, second_channels)
        self.idx[param_name + '.n2.weight'], self.idx[param_name + '.n2.bias'] = third_channels, third_channels
        self.idx[param_name + '.shortcut.weight'] = (third_channels, first_channels)
        return third_channels
...
class ResNet(nn.Module):
    def __init__(self, datashape, hidden_size, block, num_blocks, num_classes, scaler_rate=0.75, track=False):
        ...
        self.scaler = Scaler(scaler_rate)
        self.layer1 = self._make_layer(block, hidden_size[0], num_blocks[0], stride=1, scaler_rate=1,
                                       track=track)
    ...
    def get_idx_fixed_topk(self, rate, fixed_ratio=0.2, topmode='absmax'):

        start_channels = torch.arange(self.datashape[0])

        total_channels_conv = self.conv.weight.shape[0]
        fixed_count_conv = int(fixed_ratio * total_channels_conv * rate)
        topk_count_conv = int(rate * total_channels_conv) - fixed_count_conv

        all_indices_conv = torch.arange(total_channels_conv)
        fixed_channels_conv = all_indices_conv[:fixed_count_conv]


        remaining_channels_conv = all_indices_conv[fixed_count_conv:]
        conv_scores = torch.sum(self.conv.weight[remaining_channels_conv], dim=(1, 2, 3))
        topk_channels_conv = remaining_channels_conv[get_topk_index(conv_scores, topk_count_conv, topmode)]


        first_channels = torch.cat((fixed_channels_conv, topk_channels_conv))
        ...
        for s in seq_index:
            ...
            for i, blc in enumerate(eval(fun_name + s)):
                ...
                first_channels = blc.get_idx_fixed_topk(first_channels, rate, param_sub_name, fixed_ratio-0.1*int(s), topmode)
                self.idx.update(blc.idx)
    ...
    def get_idx_fixed_random(self, rate):
        start_channels = (torch.arange(self.datashape[0]))
        first_channels = torch.randperm(self.conv.weight.shape[0])[: int(rate * self.conv.weight.shape[0])]
        self.idx['conv.weight'] = (first_channels, start_channels)
        self.idx['n1.weight'], self.idx['n1.bias'] = first_channels, first_channels
        ...
        for s in seq_index:
            ...
            for i, blc in enumerate(eval(fun_name + s)):
                param_sub_name = param_name + '.' + str(i)
                first_channels = blc.get_idx_fixed_random(first_channels, rate, param_sub_name)
                self.idx.update(blc.idx)
        self.idx['linear.weight'] = (torch.arange(self.linear.weight.shape[0]), first_channels)
        self.idx['linear.bias'] = torch.arange(self.linear.weight.shape[0])
