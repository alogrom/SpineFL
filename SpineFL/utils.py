...
from models.resnet_try import resnet18, get_model_params
...
def load_params_to_client_model(global_model, client_model, inference_data, mode, device, rate, select_mode,
                                label_mask, clients_models_shape, user_idx,layer_neuron_count=None):
    ...
    if mode == 'aware':
        ...
        if layer_neuron_count is not None:
            for layer_name, indices in client_model_idx.items():
                if layer_name in layer_neuron_count:
                    if isinstance(indices, tuple):
                        selected_indices = indices[0]
                    else:
                        selected_indices = indices
                    layer_neuron_count[layer_name][selected_indices] += 1
    elif mode == 'awareGrad':
        ...
        global_model.get_idx_fixed_random(rate)
        client_model_idx = copy.deepcopy(global_model.idx)
    elif mode =='test2':
        global_model.get_idx_fixed_topk(rate,fixed_ratio=0.5,topmode=select_mode)
        client_model_idx = copy.deepcopy(global_model.idx)
        if layer_neuron_count is not None:
            for layer_name, indices in client_model_idx.items():
                if layer_name in layer_neuron_count:
        ...
        with torch.no_grad():
            inference_data = inference_data['img'].to(device)
            global_model.get_idx_scalefl(rate)
        client_model_idx = OrderedDict()
        for k, v in global_model.idx.items():
            temp = []
            if isinstance(global_model.idx[k], tuple):
                for index in range(len(global_model.idx[k])):
                    temp.append(global_model.idx[k][index].to(device))
            else:
                temp.append(global_model.idx[k].to(device))
            client_model_idx[k] = (copy.deepcopy(temp[0]), copy.deepcopy(temp[1])) if len(temp) > 1 else temp[0]
        lr = cfg['lr']
        optimizer = make_optimizer(client_model, lr)
        with torch.no_grad():
            global_predictions = global_model(inference_data)
        client_predictions = client_model(inference_data)
        kl_loss = F.kl_div(
            F.log_softmax(client_predictions, dim=-1),
            F.softmax(global_predictions, dim=-1),
            reduction='batchmean'
        )
        ...
        with torch.no_grad():
            inference_data = inference_data['img'].to(device)
            global_model.get_idx_aware(inference_data, rate, 'absmax')
            client_model_idx = OrderedDict()
            for k, v in global_model.idx.items():
                temp = []
                if isinstance(global_model.idx[k], tuple):
                    ...
                    temp.append(global_model.idx[k].to('cpu'))
                client_model_idx[k] = (copy.deepcopy(temp[0]), copy.deepcopy(temp[1])) if len(temp) > 1 else temp[0]
        optimizer = make_optimizer(client_model, cfg['lr'])
        with torch.no_grad():
            global_logits = global_model(inference_data)
        for _ in range(cfg['flexfl']['distill_epochs']):
            client_logits = client_model(inference_data)
            loss = F.kl_div(
                F.log_softmax(client_logits / cfg['flexfl']['temperature'], dim=1),
                F.softmax(global_logits / cfg['flexfl']['temperature'], dim=1),
                reduction='batchmean'
            ) * (cfg['flexfl']['temperature'] ** 2)
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(client_model.parameters(), cfg['flexfl']['grad_clip'])
            optimizer.step()
